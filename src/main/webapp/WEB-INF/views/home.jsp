            ...
            <c:forEach items="${product.content}" var="item">
                <div class="card ml-4" style="width: 18rem; margin-left: 10px; margin-bottom: 20px">
                    ...
                    <div class="card-body">
                        ...
                        <a href="/cart/add?id=${item.id}" class="btn btn-primary">Mua</a>
                    </div>
                </div>
            </c:forEach>
        </main>
        <nav aria-label="Page navigation example" class="col-3 offset-4 mt-5 mb-5">
            <ul class="pagination">
                <c:forEach end="${product.totalPages}" begin="1" var="i">
                    <li class="page-item ${i - 1 == product.number ? 'active' : ''}"><a class="page-link" href="/home?page=${i}">${i}</a></li>
                </c:forEach>
            </ul>
        </nav>
        <footer>
